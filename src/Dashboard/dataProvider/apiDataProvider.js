...
const apiDataProvider = {
  ...
  getMany: (resource, params) => {
    const endpoint = resourceMap[resource] || `/${resource}`;
    return api.get(endpoint, { params: { id: params.ids } }).then((response) => ({
      data: response.data,
    }));
  },
  ...
  update: (resource, params) => {
    ...
    if (resource === "event-planner") {
      return api.patch(`${endpoint}/${params.id}`, params.data).then((response) => ({
        data: response.data,
      }));
    }
    if (resource === "vendor") {
    ...
    if (resource === "password") {
      ...
      return api.patch(`/admin/clients/${params.data.id}/password`, { password: params.data.password }).then((response) => ({
        data: response.data,
      }));
    }
    // Default PUT
    return api.put(`${endpoint}/${params.id}`, params.data).then((response) => ({
      data: response.data,
    }));
  },
  ...
  delete: (resource, params) => {
    const endpoint = resourceMap[resource] || `/${resource}`;
    return api.delete(`${endpoint}/${params.id}`).then(() => ({ data: params.previousData }));
  },
